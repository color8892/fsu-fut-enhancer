...
npm run lint
npm run check:ea-bundle   # 對照本機 EA bundle 檢查 patch 是否仍相容
```

**EA 更新後**：從 FUT 頁 Network 重新存 `compiled_*.js`，用 `npm run check:ea-bundle -- --bundles <你的目錄>` 檢查。報告中 `✗` = 類別/方法被 EA 改掉需修 patch；確認無誤後 `npm run check:ea-bundle -- --update-baseline` 更新 `extension/data/ea-bundle-baseline.json`。
