...
**狀態**：進行中。`PatchInstaller` 已保留原有六個 phase 的順序，並新增同一實例的重複安裝保護、phase 與 feature 結果診斷、feature 級例外隔離。`tactics-role`、`navigation` 與 bootstrap 的 `home-hub.academy-tile` 已遷移為具備目標驗證、唯一 ID 與 restore 的 descriptor；其餘 patch families 仍待逐步遷移。
