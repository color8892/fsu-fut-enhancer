...
**狀態**：進行中。市場唯讀切片已建立 lazy `EaRuntimeAdapter` 與 `EaMarketSearchSession`，並遷移 UTAS session refresh、market cache、transfer market search、EA query model 和價格步進。寫入切片已涵蓋「移動物品到俱樂部」、單一及批量球員的購買後入隊交易，以及物品上架；adapter 會正規化餘額不足、拍賣到期、權限拒絕、購買後移動失敗與上架錯誤。獨立競標、未分配物品管理和其他市場寫入流程仍使用舊 EA 全域，不應被視為已完成遷移。
