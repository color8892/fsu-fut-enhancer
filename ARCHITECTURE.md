...
目前 `EaRuntimeAdapter` 已是 strict island 的一部分。`EaMarketSearchSession` 隱藏 EA search DTO/view model 的欄位同步，domain 市場唯讀流程只使用 capability、session 和結構化 failure result。寫入邊界目前正規化「移動物品到俱樂部」及單一球員購買交易；後者會等待競標和移動 observable 都結束，再把結果交回 domain 映射既有 UI。批量購買、獨立競標、上架與其他市場寫入操作尚未完成 adapter 遷移。
