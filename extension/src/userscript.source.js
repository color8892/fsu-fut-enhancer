...
(function () {
    ...
    function futweb() {
        ...
        const store = new FsuJsonStore(GM_getValue, GM_setValue);
        const httpClient = new FsuHttpClient(GM_xmlhttpRequest, navigator.userAgent);
        info = {
        ...
        //26.05 futbinId处理
        futbinId.init = () => {
            info.futbinId = store.getObject("futbinId", {});
        }
        futbinId.set = (defId, futbinId) => {
            info.futbinId[defId] = futbinId;
            store.setJson("futbinId", info.futbinId);
        }
        ...
        SBCCount.init = () => {
            let a = store.getObject("SBCCount", {}),
        ...
        lock.save = function(v){
            ...
            store.setJson("lock_26", info.lock);
        };
        build.init = () => {
            let a = store.getObject("build", {});
        ...
        build.set = (s,r) => {
            info.build[s] = r;
            console.log(info.build)
            store.setJson("build", info.build);
            events.notice(fy("notice.setsuccess"),0)
        }
