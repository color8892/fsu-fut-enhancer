...
import { OperationScope } from "../core/OperationScope.js";
 ...
 *     | "empty-plan"
 *     | "cancelled"
 *     | "virtual-challenge-unavailable"
 *     | "save-failed"
 *     | "error"
 * }} SbcTemplateLoadResult
 */
...
export class SbcTemplateService {
  ...
  async loadTemplate(controller, type, sId, helpers) {
    ...
    try {
      ...
      const conceptIndexes = _.flatMap(resultSquad, (player, index) =>
        player?.concept ? [index] : []
      );
      ...
      debug?.log?.("SBC template load failed", error);
